...
remote_state {
  ...
  config = {
    ...
    key                  = "${local.config.platform.subscriptionId}/${local.config.platform.resourceGroupName}/environments/${local.config.environment}/platform-network/terraform/state/terraform.tfstate"
  }
}
