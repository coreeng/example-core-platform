...
remote_state {
  ...
  config = {
    ...
    key            = "${local.config.platform.accountId}/environments/${local.config.environment}/platform-network/terraform/state/terraform.tfstate"
    dynamodb_table = "${local.config.platform.accountId}_environments_${local.config.environment}_platform-network_terraform_state_terraform.tfstate"
    encrypt        = true
  }
}
